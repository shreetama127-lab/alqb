...
export default function DashboardPage() {
  ...
  useEffect(() => {
    async function loadStats() {
      ...
      const { count: sCount } = await supabase.from("question_sets").select("id", { count: "exact", head: true }).eq("user_id", userData.user.id);
      if (sCount) setSetCount(sCount);

      const { data: subs } = await supabase.from("subscriptions").select("plan_id, status").eq("user_id", userData.user.id).eq("status", "active");
      const owned = (subs || []).map((s) => s.plan_id);
      setActiveIds([...new Set([...owned, ...FREE_PLAN_IDS])]);

      const { data: ans } = await supabase.from("answers").select("created_at").eq("user_id", userData.user.id);
      if (ans && ans.length > 0) {
        const days = new Set(ans.map((a) => new Date(a.created_at).toISOString().slice(0, 10)));
        let count2 = 0;
        const cursor = new Date();
        if (!days.has(cursor.toISOString().slice(0, 10))) cursor.setDate(cursor.getDate() - 1);
        while (days.has(cursor.toISOString().slice(0, 10))) { count2 += 1; cursor.setDate(cursor.getDate() - 1); }
        setStreak(count2);
      }

      const { data: exam } = await supabase.from("exam_dates").select("exam_date").eq("user_id", userData.user.id).maybeSingle();
      if (exam?.exam_date) { setExamDate(exam.exam_date); setDateInput(exam.exam_date); }
  ...
  async function saveExamDate() {
    ...
    setSavingDate(true);
    const { error } = await supabase.from("exam_dates").upsert({ user_id: userId, exam_date: dateInput, updated_at: new Date().toISOString() });
    if (error) console.error("Error saving exam date:", error);
    else { setExamDate(dateInput); setShowExamModal(false); }
    setSavingDate(false);
  }
  ...
  async function clearExamDate() {
    ...
    else { setExamDate(null); setDateInput(""); setShowExamModal(false); }
  }
  ...
  if (loading)
    return <main className="mx-auto flex min-h-[70vh] max-w-4xl items-center justify-center px-6"><p className="text-zinc-400">Loading your dashboard…</p></main>;
      ...
      {showExamModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-zinc-900/50 px-6 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-3xl bg-white p-8 shadow-2xl">
            ...
            <input type="date" value={dateInput} onChange={(e) => setDateInput(e.target.value)} className="mt-5 w-full rounded-xl border border-zinc-200 px-4 py-3 text-zinc-700 outline-none focus:border-emerald-400" />
            <div className="mt-5 flex flex-col gap-3 sm:flex-row sm:justify-end">
              {examDate && <button onClick={clearExamDate} className="rounded-full border-2 border-zinc-200 bg-white px-6 py-2.5 font-bold text-zinc-600 transition-all hover:border-red-300 hover:text-red-500">Clear</button>}
              <button onClick={saveExamDate} disabled={!dateInput || savingDate} className="rounded-full bg-emerald-700 px-8 py-2.5 font-bold text-white shadow-lg shadow-emerald-700/20 transition-all hover:-translate-y-0.5 hover:bg-emerald-800 disabled:bg-zinc-300 disabled:shadow-none">{savingDate ? "Saving…" : "Save"}</button>
            </div>
          </div>
        </div>
      )}
