...
type Question = { id: number; stem: string; options: Option[] };
...
export default function QuestionPage() {
  ...
  useEffect(() => {
    async function loadQuestions() {
      ...
      let query = supabase.from("questions").select("id, stem, options").order("id");
  ...
  async function submitAnswer() {
    ...
    setAnswers((a) => ({ ...a, [index]: { selected: pending, correct: isCorrect } }));
    ...
    if (userData.user) {
      ...
      console.error("No user logged in when submitting");
    }
  }
  ...
  function nextQuestion() {
    goToQuestion((index + 1) % questions.length);
  }
    ...
    <main className="mx-auto flex max-w-5xl flex-col gap-6 px-6 py-10 lg:flex-row">
      <div className="flex-1">
        <div className="mb-5 flex items-center justify-between rounded-2xl border border-emerald-100 bg-white p-3 shadow-sm">
          {timed ? (
            <>
              ...
              <button
                onClick={stopTimed}
                className="rounded-full border border-zinc-200 px-4 py-1.5 text-sm font-semibold text-zinc-500 transition-colors hover:bg-zinc-50"
              >
                End timer
              </button>
              <div className="text-center">
                ...
                <p className={`text-xl font-extrabold ${remaining <= 60 ? "text-red-600" : "text-emerald-700"}`}>
            ...
            <>
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  onClick={startTimed}
                  className="ml-2 rounded-full bg-emerald-700 px-5 py-2 text-sm font-bold text-white transition-colors hover:bg-emerald-800"
                >
                  Start
                </button>
              </div>
        ...
        <div className="flex items-center justify-between">
          ...
          {answeredCount > 0 && (
            <p className="rounded-full bg-emerald-100 px-4 py-1 text-sm font-bold text-emerald-800">
            ...
            {q.options.map((option) => {
              ...
              return (
                <button
                  ...
                  className={`rounded-2xl border-2 px-5 py-4 text-left text-lg transition-all ${style}`}
                >
        ...
        <div className="mt-8 flex justify-end">
          {!submitted ? (
            <button
              ...
              className="rounded-full bg-emerald-700 px-10 py-4 text-lg font-bold text-white shadow-lg shadow-emerald-700/20 transition-all hover:-translate-y-0.5 hover:bg-emerald-800 disabled:cursor-not-allowed disabled:bg-zinc-300 disabled:shadow-none"
            >
              Submit Answer
            </button>
          ) : (
            <button
              ...
              className="rounded-full bg-emerald-700 px-10 py-4 text-lg font-bold text-white shadow-lg shadow-emerald-700/20 transition-all hover:-translate-y-0.5 hover:bg-emerald-800"
            >
              Next Question →
            </button>
          )}
              ...
              return (
                <button
                  ...
                  className={`flex items-center justify-between rounded-xl px-3 py-2 text-sm transition-colors ${
                    isCurrent ? "bg-emerald-50 font-bold text-emerald-800" : "text-zinc-600 hover:bg-zinc-50"
                  }`}
                >
